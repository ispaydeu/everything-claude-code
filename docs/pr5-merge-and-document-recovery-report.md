...
|---|---|---|---|---|
| **Incorrect component counts in quickstart docs** | PRs #1–4 stated "41 commands" / "67 skills." Exact cause: `find commands/ -maxdepth 1 \| wc -l` = 41 (find counts parent dir itself); `ls -a skills/ \| wc -l` = 67 (`-a` includes `.` and `..`). Also: `README.md` lines 258–348 tree lists only 56/65 skills and 32/40 commands. Full analysis in `docs/component-count-discrepancy-analysis.md`. | Medium | **Yes — worth reporting** | File upstream issue referencing `README.md` lines 258–348 (incomplete tree). See `docs/component-count-discrepancy-analysis.md` for exact issue text. |
